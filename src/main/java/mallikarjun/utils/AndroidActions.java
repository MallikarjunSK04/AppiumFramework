...
public class AndroidActions extends AppiumUtils {
	...
	public void scrollGesture() {
		...
		do {
			isScrollable = (boolean) ((JavascriptExecutor) driver).executeScript("mobile: scrollGesture", ImmutableMap
					.of("left", 100, "top", 100, "width", 200, "height", 200, "direction", "down", "percent", 3.0));
		} while (isScrollable);
	}
	
	public void scrollGestureToText(String text) {
		driver.findElement(
	...
	public void dragGesture(WebElement ele, int xCor,int yCor) {
		((JavascriptExecutor) driver).executeScript("mobile: dragGesture", ImmutableMap.of("elementId",
				((RemoteWebElement) ele).getId(), "endX", xCor, "endY", yCor));
